        ...
        <div style={{ padding: isMobile ? "50px 20px 60px" : "80px 40px" }}>
            <div style={{ maxWidth: 1280, margin: "0 auto" }}>
                <div style={{ marginBottom: isMobile ? 36 : 56 }}>
                    <div style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: ".6rem", letterSpacing: ".38em", color: "#C8A84B", textTransform: "uppercase", marginBottom: 14 }}>What We Trade</div>
                    <h1 style={{ fontSize: isMobile ? "clamp(2rem,8vw,2.6rem)" : "clamp(2.5rem,5vw,3.8rem)", fontWeight: 300, letterSpacing: "-.02em", lineHeight: 1.1 }}>
                        Our Metals<br /><em style={{ color: "#C8A84B" }}>Portfolio</em>
                    </h1>
                </div>
                <div className="div" style={{ marginBottom: isMobile ? 28 : 52 }} />
                <p style={{ color: "#4A7A72", fontSize: ".84rem", lineHeight: 1.85, maxWidth: 700, marginBottom: isMobile ? 32 : 52 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill,minmax(300px,1fr))", gap: 2 }}>
                    {METALS.map(m => (
                        <div key={m.id} className="card" style={{ padding: isMobile ? "24px 20px" : "34px 30px" }} onClick={() => setSel(m)}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: isMobile ? 16 : 22 }}>
                                <div>
                                    <div style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: isMobile ? "1.6rem" : "2rem", letterSpacing: ".05em", color: "#C8A84B" }}>{m.name}</div>
                            ...
                            <p style={{ color: "#a4e3db", fontSize: ".83rem", lineHeight: 1.7, marginBottom: 16 }}>{m.desc}</p>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <div>
                                    <div style={{ fontSize: ".58rem", letterSpacing: ".18em", color: "#45b29e", fontFamily: "'Rajdhani',sans-serif", textTransform: "uppercase", marginBottom: 3 }}>Purity Range</div>
                                    <div style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 600, fontSize: ".83rem", color: "#E0C06A" }}>{m.grade}</div>
                                </div>
                                <span style={{ color: "#C8A84B", fontSize: ".65rem", letterSpacing: ".18em", fontFamily: "'Rajdhani',sans-serif" }}>DETAILS →</span>
                            </div>
                        </div>
                    ))}
                </div>
