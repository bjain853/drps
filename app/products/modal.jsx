    ...
    return (
        <div className="overlay">
            <div style={{ background: "#082220", border: "1px solid #1D5048", maxWidth: 560, width: "100%", padding: isMobile ? "28px 20px" : "44px", position: "relative", maxHeight: "90vh", overflowY: "auto" }}
                ...
                <button style={{ position: "absolute", top: 14, right: 18, background: "none", border: "none", color: "#2A6A60", fontSize: "1.5rem", cursor: "pointer", lineHeight: 1, padding: "4px 8px" }} onClick={() => setSel(null)}>✕</button>
                <div style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: isMobile ? "2.2rem" : "2.8rem", color: "#C8A84B", letterSpacing: ".05em" }}>{sel.symbol}</div>
                <h2 style={{ fontSize: isMobile ? "1.6rem" : "2rem", fontWeight: 300, marginBottom: 20 }}>{sel.name}</h2>
                <div className="div" style={{ marginBottom: 24 }} />
                <p style={{ color: "#7ac4b8", lineHeight: 1.9, marginBottom: 24, fontSize: ".9rem" }}>{sel.desc}</p>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 28 }}>
                    {[{ label: "Purity Grade", value: sel.grade }, { label: "Applications", value: sel.uses }].map(d => (
                ...
                <Link href={`/contact?metal=${sel.id}`}>
